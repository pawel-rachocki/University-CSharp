...
public class EmpDeptSalgradeTests
{
    ...
    public void ShouldReturnEmployeesWithCommission()
    {
        ...
        var result =  emps.Where(e => e.Comm.HasValue).Select(e => new { e.EName, e.Comm }).ToList();
    ...
    public void ShouldMatchEmployeeToSalaryGrade()
    {
        ...
        var result = emps.SelectMany(e => grades.Where(g => e.Sal >= g.Losal && e.Sal <= g.Hisal).Select(g => new { e.EName, g.Grade })).ToList();
    ...
    public void ShouldReturnEmployeesEarningMoreThanDeptAverage()
    {
        ...
        var deptAvgs = emps.GroupBy(e => e.DeptNo).ToDictionary(g => g.Key, g => g.Average(e => e.Sal));

        var result = emps.Where(e => e.Sal > deptAvgs[e.DeptNo]).Select(e => e.EName).ToList();
